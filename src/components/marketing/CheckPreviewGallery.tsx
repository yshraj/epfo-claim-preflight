...
const PREVIEWS = [
  ...
  { n: "2", status: "fail" as const },
  { n: "3", status: "pass" as const },
] as const;
            ...
            return (
              <Reveal key={p.n} delay={i * 0.08}>
                <div className={`group border border-slate-200 bg-white rounded-xl p-5 h-full transition-all duration-300 ${style.card}`}>
                  ...
                  <div className="font-medium text-sm text-slate-900">{t(`preview.${p.n}.title`)}</div>
                  <div className="text-xs text-slate-500 mt-2 transition-colors duration-300 group-hover:text-slate-700">
                    {t(`preview.${p.n}.detail`)}
                  </div>
                </div>
              </Reveal>
            );
